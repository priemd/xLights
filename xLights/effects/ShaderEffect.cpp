...
ShaderConfig::ShaderConfig(const wxString& filename, const wxString& code, const wxString& json) : _filename(filename)
{
    ...
    for (int i = 0; i < passes.Size(); i++)
    {
        _passes.push_back({
            ...
            passes[i].HasMember("WIDTH") ? passes[i]["WIDTH"].AsString() : "",
            passes[i].HasMember("HEIGHT") ? passes[i]["HEIGHT"].AsString() : ""
            });
    }
    ...
    for ( auto p : _passes )
    {
        logger_base.info("Shader - begin pass");
        logger_base.info("  target - %s", p._target.ToStdString().c_str() );
        logger_base.info("  persistent - %s", p._persistent ? "true" : "false" );
        logger_base.info("  width - %s", p._width.ToStdString().c_str() );
        logger_base.info("  height - %s", p._height.ToStdString().c_str() );
        logger_base.info("Shader - end pass");
    }
}
